...
void RCC_init(void)
{
	...
	/* disable other clocks (HSE, PLL)*/
	RCC_setClockState(RCC_CLOCK_HSE, 0);
	RCC_setClockState(RCC_CLOCK_PLL, 0);

	/* set buses prescalers */
	RCC_setBusPrescaler(RCC_BUS_AHB, RCC_AHB_PRESCALER_SYSCLK_DIV1);
	RCC_setBusPrescaler(RCC_BUS_APB1, RCC_APB_PRESCALER_HCLK_DIV1);
	RCC_setBusPrescaler(RCC_BUS_APB2, RCC_APB_PRESCALER_HCLK_DIV1);

	/* set HSE source type as external osc. */
	RCC_setHSEsourceType(RCC_HSE_SOURCE_OSCILLATOR);

	/* set PLL config: source = HSI / 2, HSE / 2 = no, multiplication = 9 */
	RCC_setPLLsource(RCC_PLL_SOURCE_HSI_DIV2);
	RCC_setPLLHSEdiv2(0);
	RCC_setPLLmul(RCC_PLL_MUL9);

	/* MCO clock source = no clock (disable MCO) */
	RCC_setMCOclock(RCC_MCO_SOURCE_NOCLOCK);
}
...
void RCC_setBusPrescaler(u8 bus, u8 prescaler)
{
	switch (bus)
	{
	case RCC_BUS_AHB:
	...
	case RCC_BUS_APB1:
		switch (prescaler)
		{
		case RCC_APB_PRESCALER_HCLK_DIV1:
		case RCC_APB_PRESCALER_HCLK_DIV2:
		case RCC_APB_PRESCALER_HCLK_DIV4:
		case RCC_APB_PRESCALER_HCLK_DIV8:
		case RCC_APB_PRESCALER_HCLK_DIV16:
